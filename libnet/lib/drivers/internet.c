...
static int do_init_channel (NET_CHANNEL *chan, int addr, int port, int forceport)
{
  ...
  getsockname (data->sock, (struct sockaddr *)&sock_addr, (socklen_t *)&addrlength);
...
static int drv_recv (NET_CHANNEL *chan, void *buf, int size, char *from)
{
  ...
  size_read = recvfrom (data->sock, (char *)buf, size, 0, (struct sockaddr *) &from_addr, (socklen_t *) &addrsize);
  if (size_read >= 0) {
